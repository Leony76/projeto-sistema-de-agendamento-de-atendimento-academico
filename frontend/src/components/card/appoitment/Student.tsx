...
const Student = (props:Props): React.JSX.Element => {
  ...
  return (
    <div className='px-3 py-2 border flex flex-col rounded-lg border-orange-300 bg-amber-50/50'>
      <div className='flex justify-between items-center'>
        <h3 className='font-bold text-orange-400'>
          { formartDateTime(props.dateTime) }
        </h3>
      ...
      <label className='text-sm text-orange-400 font-semibold'>
        Professor: <span className='text-cyan-500 font-normal'>{ props.professor }</span>
      </label>

      <label className='text-sm text-orange-400 font-semibold'>
        Status: <span className='text-cyan-500 font-normal'>{ statusMap[props.status] }</span>
      </label>
